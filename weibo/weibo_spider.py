...
# --------------------------------------------文件存储-----------------------------------------------------
path = os.getcwd() + "/data/weibo/weiboComments.csv"
...
def get_title_id():
    for page in range(1, 3):  # 每个页面大约有18个话题
        headers = {
            # "User-Agent" : UserAgent().chrome #chrome浏览器随机代理
            "User-Agent": ua.random
        }
        time.sleep(1)
        api_url = 'https://m.weibo.cn/api/container/getIndex?containerid=231522type%3D60%26q%3D%23%E5%90%8C%E5%BF%83%E6%8A%97%E7%96%AB%23%26t%3D10&extparam=%23%E5%90%8C%E5%BF%83%E6%8A%97%E7%96%AB%23&luicode=10000011&lfid=231522type%3D1%26amp%3Bt%3D10%26amp%3Bq%3D%23%E7%96%AB%E6%83%85%23&page_type=searchall&page=' + str(
            page)
        print(api_url)
        ...
        for json in rep.json()['data']['cards'][1:]:
            # print(json)
            if json['card_type'] == 9:
...
def spider_title(comment_ID):
    ...
    position1 = (
    article_url, title_text, title_user_id, title_user_NicName, title_user_gender, "", add_title_time, reposts_count,
